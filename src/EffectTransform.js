...
class EffectTransform {
    ...
    static transformColor (drawable, inOutColor, effectMask) {
        ...
        if (typeof effectMask === 'number') effectBits &= effectMask;
        const effects = drawable._effects;
        ...
        if ((effectBits & EffectManager.EFFECT_INFO.ghost.mask) !== 0) {
            ...
            inOutColor[0] *= effects.ghost;
            inOutColor[1] *= effects.ghost;
            inOutColor[2] *= effects.ghost;
            inOutColor[3] *= effects.ghost;
        }
    ...
    static transformPoint (drawable, vec, dst) {
        ...
        const effectBits = drawable.enabledEffects;
        const effects = drawable._effects;
        if ((effectBits & EffectManager.EFFECT_INFO.mosaic.mask) !== 0) {
            // texcoord0 = fract(u_mosaic * texcoord0);
            dst[0] = effects.mosaic * dst[0] % 1;
            dst[1] = effects.mosaic * dst[1] % 1;
        }
        if ((effectBits & EffectManager.EFFECT_INFO.pixelate.mask) !== 0) {
            // vec2 pixelTexelSize = u_skinSize / u_pixelate;
            // TODO: is skin.size correct?
            const texelX = drawable.skin.size[0] / effects.pixelate;
            const texelY = drawable.skin.size[1] / effects.pixelate;
        ...
        if ((effectBits & EffectManager.EFFECT_INFO.whirl.mask) !== 0) {
            ...
            const whirlActual = effects.whirl * whirlFactor * whirlFactor;
        ...
        if ((effectBits & EffectManager.EFFECT_INFO.fisheye.mask) !== 0) {
            ...
            const r = Math.pow(Math.min(vLength, 1), effects.fisheye) * Math.max(1, vLength);
